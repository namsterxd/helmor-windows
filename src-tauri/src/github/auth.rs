...
pub struct GithubIdentityFlowRuntime {
    ...
    /// CSRF `state` parameter generated by `start_github_oauth_redirect`.
    /// Consumed and cleared by `handle_oauth_callback_from_url`.
    pending_oauth_state: Arc<Mutex<Option<String>>>,
}
...
/// Returned by `start_github_oauth_redirect`. The frontend opens this URL in
/// the default browser to initiate the authorization code flow.
#[derive(Debug, Clone, Serialize, PartialEq, Eq)]
#[serde(rename_all = "camelCase")]
pub struct GithubOAuthRedirectStart {
...
// ---------------------------------------------------------------------------
// OAuth authorization code redirect flow (primary login path)
//
// Uses a temporary localhost HTTP server to receive the callback rather than
// a custom URI scheme, because `cargo tauri dev` runs a raw binary (not a
// `.app` bundle) and macOS won't route `helmor-dev://` to it. Localhost
// works everywhere: dev, release, all platforms.
// ---------------------------------------------------------------------------

fn github_client_secret() -> Option<&'static str> {
    option_env!("HELMOR_GITHUB_CLIENT_SECRET").filter(|v| !v.trim().is_empty())
}

/// Generate the GitHub OAuth authorization URL, spin up a one-shot localhost
/// server to receive the callback, and spawn a background thread that waits
/// for it. When GitHub redirects the browser to `http://127.0.0.1:<port>/`,
/// the server extracts the `code` + `state`, exchanges the code for an
/// access token, persists the identity, and emits a snapshot event.
pub fn start_github_oauth_redirect(
    app: AppHandle,
    runtime: GithubIdentityFlowRuntime,
) -> Result<GithubOAuthRedirectStart> {
    let client_id = require_client_id(github_client_id())?;
    let client_secret = github_client_secret()
        .with_context(|| {
            "HELMOR_GITHUB_CLIENT_SECRET is not configured. \
             Rebuild Helmor with this env var set."
        })?
        .to_string();

    // Bind to an ephemeral port on loopback.
    let listener = std::net::TcpListener::bind("127.0.0.1:0")
        .context("Failed to bind localhost OAuth callback server")?;
    let port = listener.local_addr()?.port();
    let redirect_uri = format!("http://127.0.0.1:{port}");

    let state = uuid::Uuid::new_v4().to_string();
    *runtime.pending_oauth_state.lock().unwrap() = Some(state.clone());
    ...
    thread::spawn(move || {
        // Set a generous timeout so the listener doesn't block forever if
        ...
        let _ = listener.incoming().next().and_then(|stream| {
            ...
            let query_str = request
                ...
                .nth(1)?
                .strip_prefix("/")?;
            let fake_url = url::Url::parse(&format!("http://localhost/{query_str}")).ok()?;

            let code = fake_url
                .query_pairs()
                .find(|(k, _)| k == "code")
                .map(|(_, v)| v.to_string());
            let callback_state = fake_url
                .query_pairs()
                .find(|(k, _)| k == "state")
                .map(|(_, v)| v.to_string());

            // Respond immediately so the browser tab shows a nice message.
            let body =
                "<html><body style=\"font-family:system-ui;text-align:center;padding:60px\">\
                     <h2>Authorization successful</h2>\
                     <p>You can close this tab and return to Helmor.</p>\
                     </body></html>";
            let response = format!(
                "HTTP/1.1 200 OK\r\n\
            ...
            let snapshot = exchange_oauth_callback(
                &client_id_owned,
                &client_secret,
                &redirect_uri,
                code,
            ...
            let _ = emit_github_identity_snapshot(&app, &snapshot);

            Some(())
        });
    });

    Ok(GithubOAuthRedirectStart {
        oauth_url: oauth_url.to_string(),
    })
}

/// Shared logic: verify CSRF state, exchange authorization code for token,
/// resolve user profile, persist identity.
fn exchange_oauth_callback(
    client_id: &str,
    client_secret: &str,
    redirect_uri: &str,
    code: Option<String>,
    ...
    let Some(code) = code else {
        return GithubIdentitySnapshot::Error {
            message: "OAuth callback missing authorization code.".to_string(),
        };
    };

    // CSRF check
    let expected_state = runtime.pending_oauth_state.lock().unwrap().take();
    match (&expected_state, &callback_state) {
        ...
        (None, _) => {
            return GithubIdentitySnapshot::Error {
                message: "No pending OAuth flow. Please try signing in again.".to_string(),
            };
        }
        (Some(_), None) => {
            return GithubIdentitySnapshot::Error {
    ...
    let result = (|| -> Result<GithubIdentitySnapshot> {
        let client = ReqwestGithubClient::new()?;

        let token_response =
            client.exchange_authorization_code(client_id, client_secret, &code, redirect_uri)?;
        ...
        let secret = StoredIdentitySecret {
            ...
            access_token_expires_at: token_response.expires_in.map(expiry_from_now),
            refresh_token_expires_at: token_response.refresh_token_expires_in.map(expiry_from_now),
        };
        let meta = GithubIdentityMeta {
            provider: "github-oauth-redirect".to_string(),
    ...
    match result {
        ...
        Err(error) => GithubIdentitySnapshot::Error {
            message: format!("{error:#}"),
        },
    }
}
...
mod tests {
    ...
    impl GithubHttpClient for MockGithubClient {
        ...
        fn exchange_authorization_code(
            ...
            unimplemented!("authorization code exchange not mocked")
        }
