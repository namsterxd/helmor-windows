...
async function openGithubMenu() {
  const trigger = await screen.findByRole("button", { name: "GitHub account menu" });
...
describe("App GitHub identity states", () => {
  let identityListener:
    | ((snapshot: { status: string; session?: typeof CONNECTED_IDENTITY }) => void)
    | null;
  ...
  beforeEach(() => {
    ...
    apiMocks.loadGithubIdentitySession.mockResolvedValue({ status: "disconnected" });
    ...
    apiMocks.listenGithubIdentityChanged.mockImplementation(async (callback) => {
  ...
  it("renders the shell while GitHub account is disconnected", async () => {
    ...
    expect(await screen.findByRole("main", { name: "GitHub identity gate" })).toBeInTheDocument();
    expect(screen.queryByRole("main", { name: "Application shell" })).not.toBeInTheDocument();
  ...
  it("connects GitHub identity and shows the account menu", async () => {
    ...
    expect(await screen.findByRole("main", { name: "Application shell" })).toBeInTheDocument();
  ...
  it("uses a compact GitHub trigger in the toolbar", async () => {
    ...
    await screen.findByRole("main", { name: "Application shell" });
    expect(screen.getByRole("button", { name: "GitHub account menu" })).toHaveTextContent(
      "caspian",
    );
  });
});
